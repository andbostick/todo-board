...
function Card({ list, index, addTodoDocument, todo, id, db }) {
  ...
  return (
    //main container
    <div
      className="relative bg-white rounded-md p-3 m-3 justify-items-center"
      key={index}
    >
      {/*delete icon*/}
      <button className="absolute -top-1 right-0" onClick={deleteTodo}>
        <FiTrash />
      </button>
      ...
      <div className="grid m-3 place-items-center gap-4">
        ...
        <form onSubmit={handleSubmit}>
          ...
          />
          <button
            className="shadow m-1 rounded-full bg-blue-400 p-1 hover:bg-blue-600"
            type="submit"
          >
            Enter
          </button>
        ...
        <div className="w-full divide-y divide-black divide-opacity-25">
